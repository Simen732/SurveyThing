// Survey editor functionality for editing existing surveys
...
function loadExistingSurvey() {
    ...
    try {
        ...
        if (surveyData.questions && surveyData.questions.length > 0) {
            surveyData.questions.sort((a, b) => a.order - b.order).forEach(question => {
        ...
        addQuestion(); // Add empty question as fallback
    }
}
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <textarea class="form-control question-text" rows="2" 
                              placeholder="Enter your question..." required>${questionData.text}</textarea>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select question-type" onchange="updateQuestionOptions(${questionCounter})" required>
                        ...
                        <option value="rating" ${questionData.type === 'rating' ? 'selected' : ''}>Rating Scale (1-5 stars)</option>
                        <option value="multiple-choice" ${questionData.type === 'multiple-choice' ? 'selected' : ''}>Multiple Choice</option>
                    </select>
                </div>
                
                <div class="options-container" id="options-${questionCounter}" style="display: ${questionData.type === 'multiple-choice' ? 'block' : 'none'};">
                    <label class="form-label">Answer Options</label>
                    <div class="options-list">
                        ${questionData.type === 'multiple-choice' && questionData.options ? 
                          questionData.options.map((option, index) => `
                            <div class="input-group mb-2">
                                <input type="text" class="form-control option-text" placeholder="Option ${index + 1}" value="${option.text}" ${questionData.type === 'multiple-choice' ? 'required' : ''}>
